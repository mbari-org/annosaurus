...
//  private[this] val log = LoggerFactory.getLogger(getClass)
  ...
  it should "create using existing recorded timestamp" in {
    val a = exec(() =>
      controller
        .create(UUID.randomUUID(), "Grimpoteuthis", "brian", recordedDate = Some(recordedDate))
    )
  ...
  it should "stream by videoReferenceUuid and timestamps" in {
    ...
    val (closeable, stream) = controller.streamByVideoReferenceUUIDAndTimestamps(
      videoReferenceUuid,
      recordedDate,
  ...
  it should "stream by concurrent request" in {
    ...
    val xs = for {
      ...
      exec(() => controller.create(uuid, "Sharktopod", "brian", recordedDate = Some(Instant.now())))
    }
    ...
    val (closeable2, stream2) =
      controller.streamByConcurrentRequest(concurrentRequest, Some(10), Some(2))
  ...
  it should "stream by multi request" in {
    ...
    val xs = for {
      ...
      exec(() => controller.create(uuid, "Octosaurus", "brian", recordedDate = Some(Instant.now())))
    }
  ...
  it should "create and update with different elapsed times" in {
    ...
    val a = exec(() =>
      controller.create(UUID.randomUUID(), "Grimpoteuthis", "brian", elapsedTime = Some(et0))
    )
    a.concept should be("Grimpoteuthis")
    a.observer should be("brian")
  ...
  it should "bulk insert complex annotations" in {
    val videoReferenceUuid = UUID.randomUUID()
    val now                = Instant.now()
    val imagedMoments = (1 until 3).map(i =>
  ...
  it should "bulk insert a full dive of annotations" in {
    ...
    val deleteCount =
      exec(() => imagedMomentController.deleteByVideoReferenceUUID(videoReferenceUuid))
