...
class CachedAncillaryDatumEndpoints(controller: CachedAncillaryDatumController)(using
    ...
    val findDataByVideoReferenceUuidImpl: ServerEndpoint[Any, Future] =
        findDataByVideoReferenceUuid
            .serverLogic { uuid => handleErrors(controller.findByVideoReferenceUUID(uuid).map(_.map(_.toSnakeCase))) }

    // GET /imagedmoment/:uuid
    val findDataByImagedMomentUuid: Endpoint[Unit, UUID, ErrorMsg, CachedAncillaryDatumSC, Any] = openEndpoint
        .get
    ...
    val findDataByImagedMomentUuidImpl: ServerEndpoint[Any, Future] =
        findDataByImagedMomentUuid
            .serverLogic { uuid => handleOption(controller.findByImagedMomentUUID(uuid).map(_.map(_.toSnakeCase))) }

    // GET /observation/:uuid
    val findDataByObservationUuid: Endpoint[Unit, UUID, ErrorMsg, CachedAncillaryDatumSC, Any] = openEndpoint
        .get
    ...
    val findDataByObservationUuidImpl: ServerEndpoint[Any, Future] =
        findDataByObservationUuid
            .serverLogic { uuid => handleOption(controller.findByObservationUUID(uuid).map(_.map(_.toSnakeCase))) }

    // POST / form or json body
    val createOneDatum: Endpoint[Option[String], CachedAncillaryDatumSC, ErrorMsg, CachedAncillaryDatumSC, Any] = secureEndpoint
        .post
        .in("v1" / "ancillarydata")
    ...
    val createDataImpl: ServerEndpoint[Any, Future] =
        createOneDatum
            ...
            .serverLogic { - => data =>
                handleOption(
                    controller.create(data.toCamelCase)
                        .map(_.map(_.toSnakeCase)))}

    // POST /bulk json body
    val createOrUpdateManyData: Endpoint[Option[String], Seq[CachedAncillaryDatumSC], ErrorMsg, Seq[CachedAncillaryDatumSC], Any] = secureEndpoint
        .post
    ...
    val createOrUpdateManyDataImpl: ServerEndpoint[Any, Future] =
        createOrUpdateManyData
            ...
            .serverLogic { - => data =>
                handleErrors(
                    controller.bulkCreateOrUpdate(data.map(_.toCamelCase))
                        .map(_.map(_.toSnakeCase)))}


    // PUT /merge/:uuid json body
    val mergeManyData: Endpoint[Option[String], (UUID, Seq[CachedAncillaryDatumSC], Option[Int]), ErrorMsg, Seq[CachedAncillaryDatumSC], Any] = secureEndpoint
        .put
    ...
    val mergeManyDataImpl: ServerEndpoint[Any, Future] =
        mergeManyData
            ...
            .serverLogic { - => (uuid, data, window) =>
                ...
                handleErrors(
                    controller.merge(data.map(_.toCamelCase), uuid, tolerance)
                        .map(_.map(_.toSnakeCase)))
            }

    // PUT /:uuid form or json body
    val updateOneDatum: Endpoint[Option[String], (UUID, CachedAncillaryDatumSC), ErrorMsg, CachedAncillaryDatumSC, Any] = secureEndpoint
        .put
        .in("v1" / "ancillarydata" / path[UUID]("uuid"))
        .in(oneOfBody(jsonBody[CachedAncillaryDatumSC], formBody[CachedAncillaryDatumSC]))
        .out(jsonBody[CachedAncillaryDatumSC])
    ...
    val updateOneDatumImpl: ServerEndpoint[Any, Future] =
        updateOneDatum
            ...
            .serverLogic { - => (uuid, data) =>
                handleOption(
                    controller.update(uuid, data.toCamelCase)
                        .map(_.map(_.toSnakeCase)))}
