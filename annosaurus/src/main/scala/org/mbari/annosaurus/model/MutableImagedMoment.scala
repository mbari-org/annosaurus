...
trait MutableImagedMoment extends PersistentObject {
    ...
    /** videoReferenceUUID is a UUID that could point to a video-reference in the
      * `video-asset-manager`. In turn, that could be used to reference another version of the same
      * video segment. Alternately, this could also point at __any__ UUID in any MAM system. Exactly
      * how it is used is not hard-coded.
      */
