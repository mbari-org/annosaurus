...
import org.mbari.annosaurus.model.simple.{ErrorMsg, ObservationCount}
...
class ObservationV1Api(controller: ObservationController)(implicit val executor: ExecutionContext)
            ...
            .map {
                case None    =>
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(404, "An ImagedMoment with a UUID of $uuid was not found")
                            )
                        )
                    )
    ...
    get("/association/:uuid") {
        val uuid =
            params
                .getAs[UUID]("uuid")
                .getOrElse(
                    halt(BadRequest(toJson(ErrorMsg(400, "Please provide an Association UUID"))))
                )
        controller
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"No observation for association with uuid of ${uuid} was found"
                                )
                            )
                        )
                    )
    ...
    get("/concepts/:uuid") {
        ...
        controller
            .findAllConceptsByVideoReferenceUUID(uuid)
            .map(_.toArray)
            .map(toJson)
    }
    ...
    get("/concept/count/:concept") {
        val concept =
            params
                ...
                .getOrElse(
                    halt(
                        BadRequest(toJson(ErrorMsg(400, "Please provide a concept to search for")))
                    )
                )
        controller
        ...
        val oldConcept =
            params
                ...
                .getOrElse(
                    halt(
                        BadRequest(
                            toJson(ErrorMsg(400, "Please provide the concept being replaced"))
                        )
                    )
                )
        val newConcept =
            params
                ...
                .getOrElse(
                    halt(
                        BadRequest(toJson(ErrorMsg(400, "Please provide the replacement concept")))
                    )
                )
        controller
    ...
    put("/:uuid") {
        ...
        val observationDate  =
            params.getAs[Instant]("observation_timestamp").getOrElse(Instant.now())
        ...
        controller.deleteDuration(uuid) map {
            case None      =>
                halt(
                    NotFound(
                        toJson(
                            ErrorMsg(404, s"Failed. No observation with UUID of $uuid was found.")
                        )
                    )
                )
            case Some(obs) => toJson(obs)
        ...
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                if (uuids == null || uuids.isEmpty)
                    halt(
                        BadRequest(
                            toJson(ErrorMsg(400, "No observation UUIDs were provided as JSON"))
                        )
                    )
                ...
                halt(
                    BadRequest(
                        toJson(
                            ErrorMsg(
                                400,
                                "bulk delete only accepts JSON (Content-Type: application/json)"
                            )
                        )
                    )
                )
