...
/** All Api classes should mixin this trait. It defines the common traits used by all
  * implementations as well implicits need for type conversions.
  *
  * @author
  *   Brian Schlining
...
abstract class APIStack extends ScalatraServlet with ApiAuthenticationSupport with FutureSupport {
    ...
    protected[this] val compactTimeFormatter1 =
        DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmss.SSSX")
    protected[this] val compactTimeFormatter2 =
        DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmss.SSSSSSX")
    ...
    implicit val stringToInstant: TypeConverter[String, Instant] =
        new TypeConverter[String, Instant] {
            override def apply(s: String): Option[Instant] = {
                val try1 = Try(Instant.from(compactTimeFormatter.parse(s))).toOption
                val try2 = try1 match {
                    case Some(_) => try1
    ...
    implicit val stringToDuration: TypeConverter[String, time.Duration] =
        new TypeConverter[String, Duration] {
            override def apply(s: String): Option[Duration] = Try(
                Duration.ofMillis(s.toLong)
            ).toOption
        }
    ...
    implicit val stringToTimecode: TypeConverter[String, Timecode] =
        new TypeConverter[String, Timecode] {
    ...
    /** "Stream" the iterable data as a chunked response.
      ...
      * @param items
      *   The items to stream back
      * @tparam T
      *   The type of each item in items
      */
    ...
    /** Pages data and sends each page back as a chunked response to the server
      ...
      *   The response object
      * @param start
      *   The starting index of the data to stream (same as offset)
      * @param end
      *   The ending index of the data to stream (could be offset + limit)
      * @param pageSize
      *   The number of items in each chunked page
      * @param fn
      *   A function that takes (limit, offset) and fetches a iterable of items (as a future). This
      *   function is fetching a page of data from the database.
      * @param timeout
      *   The timeout for a chunk.
      * @tparam T
      *   The type of items returned from the database.
      */
    def autoPage[T](
