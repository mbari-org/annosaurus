                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"an Image with an image_reference_uuid of $uuid was not found"
                                )
                            )
                        )
                    )
    ...
    get("/name/:name") {
        val name = params
            ...
            .getOrElse(
                halt(
                    BadRequest(
                        toJson(ErrorMsg(400, "An image name is required as part of the path"))
                    )
                )
            )
        controller
...
//       )))
        ...
        controller
            ...
            .map {
                case None    =>
                    halt(
                        NotFound(toJson(ErrorMsg(404, "an Image with a URL of $url was not found")))
                    )
