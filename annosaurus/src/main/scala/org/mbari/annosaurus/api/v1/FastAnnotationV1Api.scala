...
class FastAnnotationV1Api(daoFactory: JPADAOFactory)(implicit val executor: ExecutionContext)
                            ...
                            halt(
                                BadRequest(
                                    toJson(
                                        ErrorMsg(
                                            404,
                                            "Range was not found for annotations matching your query"
                                        )
                                    )
                                )
                            )
    ...
    get("/toconcept/images/:toconcept") {
        val concept =
            params
                .get("toconcept")
                .getOrElse(
                    halt(BadRequest(toJson(ErrorMsg(400, "A 'toconcept' parameter is required"))))
                )
        val limit   = params.getAs[Int]("limit")
        val offset  = params.getAs[Int]("offset")
        val addData = params.getAs[Boolean]("data").getOrElse(false)
    ...
    get("/details/:link_name/:link_value") {
        ...
        val linkValue = params
            .get("link_value")
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "A link_value parameter is required"))))
            )
        val addData   = params.getAs[Boolean]("data").getOrElse(false)
        Future {
        ...
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                Future {
                    val annos =
                        repository
                            .findByConcurrentRequest(concurrentRequest, limit, offset, addData)
                            .asJava
                    toJson(annos)
                }
            case _                  =>
    ...
    post("/multi") {
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                Future {
                    val annos =
                        repository.findByMultiRequest(multiRequest, limit, offset, addData).asJava
                    toJson(annos)
                }
            case _                  =>
                halt(
