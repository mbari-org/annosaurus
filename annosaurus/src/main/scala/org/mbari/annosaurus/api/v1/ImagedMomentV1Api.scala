...
import org.mbari.annosaurus.model.simple.{Count, ErrorMsg, ObservationCount, WindowRequest}
...
class ImagedMomentV1Api(controller: ImagedMomentController)(implicit val executor: ExecutionContext)
    ...
    get("/count/images/:video_reference_uuid") {
        val videoReferenceUuid = params
            ...
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide a video_reference_uuid"))))
            )
        controller
    ...
    get("/imagereference/:uuid") {
        val uuid = params
            .getAs[UUID]("uuid")
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide an ImageReference UUID"))))
            )
        controller
            ...
            .map {
                case None     =>
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(404, s"No imagereference with a uuid of $uuid was found")
                            )
                        )
                    )
        ...
        val uuid =
            params
                ...
                .getOrElse(
                    halt(
                        BadRequest(
                            toJson(ErrorMsg(400, "Please provide an MutableObservation UUID"))
                        )
                    )
                )
        controller
            ...
            .map {
                case None     =>
                    halt(
                        NotFound(
                            toJson(ErrorMsg(404, s"No observation with a uuid of $uuid was found"))
                        )
                    )
                case Some(im) => toJson(im)
            }
    ...
    put("/newtime/:uuid/:time") {
        ...
        val uuid = params
            ...
            .getOrElse(
                halt(
                    BadRequest(
                        toJson(ErrorMsg(400, "Please provide a video reference UUID parameter"))
                    )
                )
            )
        val time = params
            ...
            .getOrElse(
                halt(
                    BadRequest(toJson(ErrorMsg(400, "Please provide a new start 'time' parameter")))
                )
            )
        controller
    ...
    put("/tapetime") {
        ...
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                val count =
                    Map("annotation_count" -> annotations.size, "timestamps_updated" -> n).asJava
                toJson(count)
            case _                  =>
                halt(
                    BadRequest(
    ...
    delete("/:uuid") {
        ...
        val uuid = params
            ...
            .getOrElse(
                halt(
                    BadRequest(
                        toJson(ErrorMsg(400, "Please provide the 'uuid' of the association"))
                    )
                )
            )
        controller
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"Failed. No ImagedMoment with UUID of $uuid was found."
                                )
                            )
                        )
                    )
