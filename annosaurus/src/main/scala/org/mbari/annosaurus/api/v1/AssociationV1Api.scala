...
class AssociationV1Api(controller: AssociationController)(implicit val executor: ExecutionContext)
    ...
    get("/:uuid") {
        val uuid =
            params
                .getAs[UUID]("uuid")
                .getOrElse(
                    halt(BadRequest(toJson(ErrorMsg(400, "Please provide a 'uuid' parameter"))))
                )
        controller
            .findByUUID(uuid)
            .map {
                case None    =>
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(400, s"An Association with a UUID of $uuid was not found")
                            )
                        )
                    )
    ...
    post("/") {
        ...
        val uuid            = params
            ...
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide an 'observation_uuid'"))))
            )
        val linkName        =
            params
                .get("link_name")
        ...
        controller
            .create(uuid, linkName, toConcept, linkValue, mimeType, associationUuid)
            .map(toJson)
    }
        ...
        controller
            ...
            .map {
                case None    =>
                    halt(
                        NotFound(
                            toJson(ErrorMsg(404, s"No association with uuid of $uuid was found"))
                        )
                    )
        ...
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                if (uuids == null || uuids.isEmpty)
                    halt(
                        BadRequest(
                            toJson(ErrorMsg(404, "No association UUIDs were provided as JSON"))
                        )
                    )
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    412,
                                    s"Failed. No association with UUID of $uuid was found."
                                )
                            )
                        )
                    )
