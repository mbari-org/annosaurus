...
class AnnotationV1Api(controller: AnnotationController)(implicit val executor: ExecutionContext)
    ...
    get("/:uuid") {
        ...
        controller
            ...
            .map {
                case None      =>
                    halt(NotFound(body = s"An observation with uuid of $uuid was not found"))
    ...
    post("/multi") {
        request.getHeader("Content-Type") match {
            case "application/json" =>
                ...
                val multiRequest        = fromJson(b, classOf[MultiRequest])
                val (closeable, stream) =
                    controller.streamByMultiRequest(multiRequest, limit, offset)
                ResponseUtilities.sendStreamedResponse(
                    response,
                    stream,
                    (a: MutableAnnotation) => toJson(a)
    ...
    get("/imagereference/:uuid") {
        val uuid = params
            ...
            .getOrElse(
                halt(
                    BadRequest(
                        toJson(ErrorMsg(400, "A image reference 'uuid' parameter is required"))
                    )
                )
            )
        controller
    ...
    post("/") {
        ...
        val concept            =
            params
                .get("concept")
                .getOrElse(
                    halt(BadRequest(toJson(ErrorMsg(400, "A 'concept' parameter is required"))))
                )
        val observer           =
            params
                .get("observer")
                .getOrElse(
                    halt(BadRequest(toJson(ErrorMsg(400, "An 'observer' parameter is required"))))
                )
        val observationDate    =
    ...
    put("/:uuid") {
        ...
        val uuid               = params
            ...
            .getOrElse(
                halt(
                    BadRequest(toJson(ErrorMsg(400, "An observation 'uuid' parameter is required")))
                )
            )
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"An annotation with observation_uuid of $uuid was not found"
                                )
                            )
                        )
                    )
