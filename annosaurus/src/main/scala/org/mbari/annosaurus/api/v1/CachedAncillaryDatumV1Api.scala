...
import org.mbari.annosaurus.model.simple.{CachedAncillaryDatumBean, ErrorMsg}
...
/** @author
  ...
  */
class CachedAncillaryDatumV1Api(controller: CachedAncillaryDatumController)(implicit
    ...
    get("/:uuid") {
        val uuid = params
            .getAs[UUID]("uuid")
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide a ancillary data UUID"))))
            )
        controller
            .findByUUID(uuid)
            .map {
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"An AncillaryDatum with a UUID of $uuid was not found"
                                )
                            )
                        )
                    )
    ...
    get("/imagedmoment/:uuid") {
        val uuid = params
            .getAs[UUID]("uuid")
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide an image reference UUID"))))
            )
        controller
    ...
    get("/observation/:uuid") {
        val uuid = params
            .getAs[UUID]("uuid")
            .getOrElse(
                halt(BadRequest(toJson(ErrorMsg(400, "Please provide an observation UUID"))))
            )
        controller
    ...
    put("/merge/:uuid") {
        ...
        val uuid         = params
            ...
            .getOrElse(
                halt(
                    BadRequest(
                        toJson(ErrorMsg(400, "A video reference 'uuid' parameter is required"))
                    )
                )
            )
                    ...
                    halt(
                        NotFound(
                            toJson(
                                ErrorMsg(
                                    404,
                                    s"A CachedAncillaryDatum with uuid of $uuid was not found"
                                )
                            )
                        )
                    )
