...
trait AssociationDAOITSuite extends BaseDAOSuite {
    ...
    test("findByLinkNameAndVideoReferenceUUIDAndConcept") {
        ...
        val a = o.getAssociations.asScala.head
        given dao: AssociationDAOImpl = daoFactory.newAssociationDAO()
        val ys = run(() => dao.findByLinkNameAndVideoReferenceUUIDAndConcept(a.getLinkName, i.getVideoReferenceUuid))
        assert(ys.size == 1)
        AssertUtils.assertSameAssociation(a, ys.head)

        val zs = run(() => dao.findByLinkNameAndVideoReferenceUUIDAndConcept(a.getLinkName, i.getVideoReferenceUuid, Some(o.getConcept())))
        assert(zs.size == 1)
    ...
    test("create") {
        ...
        run(() => {
            obsDao.findByUUID(o.getUuid()).foreach(obs => {
    ...
    test("delete") {
        ...
        val a = o.getAssociations().asScala.head
        given dao: AssociationDAOImpl = daoFactory.newAssociationDAO()
        run(() => dao.findByUUID(a.getUuid()) match {
