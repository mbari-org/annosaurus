...
class CachedVideoReferenceInfoV1Api(controller: CachedVideoReferenceInfoController)(
  ...
  get("/:uuid") {
    ...
    controller
      ...
      .map({
        case None =>
          halt(NotFound(body = s"A CachedVideoReferenceInfo with a UUID of $uuid was not found"))
  ...
  post("/") {
    ...
    val missionID =
      params.get("mission_id").getOrElse(halt(BadRequest("A 'mission_id' parameter is required")))
