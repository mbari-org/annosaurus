...
class ImagedMomentDAOSpec extends FlatSpec with Matchers {
  ...
  it should "update" in {
    ...
    val imagedMoment = run(dao.runTransaction(d => d.findByVideoReferenceUUID(videoReferenceUUID))).head
  ...
  it should "findAll" in {
    val imNew = ImagedMomentImpl(Some(videoReferenceUUID), elapsedTime = Some(Duration.ofSeconds(1345)))
    run(dao.runTransaction(d => d.create(imNew)))
  ...
  it should "delete" in {
    val im = run(dao.runTransaction(d => d.findAll()))
    run(dao.runTransaction(d => im.foreach(d.delete)))
    val imCheck = run(dao.runTransaction(d => d.findAll()))
